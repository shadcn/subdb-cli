  ...
  exports.getHash(file_path, function (hash) {
    if (hash) {
      ...
      exports.call(options, function(error, response, body) {
        ...
        // Subdb sends iso-8859-15 subtitles as utf8, we have to convert them
        var encoding = language === 'en' ? 'utf8' : 'iso-8859-15';
        var decodedBody = iconv.decode(body, encoding);

        // Write to a file if success.
        if (!error && response.statusCode == 200) {
          fs.writeFile(dir + '/' + filename + '.srt', decodedBody, 'utf8', function (err) {
...
exports.call = function(params, callback) {
  var options = {
    ...
    headers: {
      'User-Agent': 'SubDB/1.0 (Subdb/1.0.1; http://github.com/arshad/subdb-cli)'
    }
  };
  ...
  // Save as buffer for later decoding
